...
struct Phone
{
    char description[PHONE_DESC_LEN + 1]; // +1 for the null terminator
    char number[PHONE_LEN + 1]; // +1 for the null terminator
};
...
{
    int patientNumber;
    char name[NAME_LEN + 1]; // +1 for the null terminator
    struct Phone phone;
};
...
// Check if a time slot is available for a given date and time.
int timeSlotAvailable(struct Date date, struct Time time, struct Appointment* app, int maxAppointments);

// Find the next available time slot in the appointments array.
int nextSlotAvailable(struct Appointment* app, int maxAppointments);

//Check if a given patient can have a valid appointment on a specific date.
